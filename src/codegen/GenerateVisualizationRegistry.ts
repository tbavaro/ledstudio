...
const TEMPLATE_FILE = "src/codegen/Visualizations.ts.template";
...
crawl(VISUALIZATION_ROOT, "", (relPath: string, fileName: string) => {
  ...
  const visualizationName = fileNameWithoutExtension.replace(
    /Visualization$/i,
    ""
  );
...
const template = handlebars.compile(
  fs.readFileSync(TEMPLATE_FILE, { encoding: "utf8" })
);
